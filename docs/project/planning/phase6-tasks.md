...
- [ ] Add MCP tool `get_code_diagnostics(path: string)` for Coder Tenant
- [ ] Add MCP tool `get_symbol_definition(path: string, symbol: string)` for Coder Tenant
- [ ] Test: Coder Tenant calls `get_code_diagnostics` → receives actual compilation errors

### 6.2 Advanced Retrieval
- [ ] Improve `ArchiveManager.SearchSimilar()` with metadata filtering
- [ ] Add filters: `tenant_type`, `outcome` (Success/Failed), `date_range`
- [ ] Example query: "Find failed Backend Tenant episodes from last 30 days"
...
- [ ] Test: ask Alfred about "auth system" → retrieves specific design doc + related failed attempts
...
- [ ] Add progress bar (`bubbles/progress`) for long-running Tenant tasks
...
- [ ] Create demo GIF showing full loop: Idea → Alfred → Tenant → Filing
...
**Phase 6 Definition of Done:** The application is feature-complete, visually polished, handles terminal resizing gracefully, and Tenants can read actual codebase errors via LSP.
